...
public class AdsServiceProducerTest {
	...
	public void testScpServiceFromVcapServices() {
		final ScpCfService scpService = new AdsServiceProducer().getScpAdsService();
	...
	public void testProducerMehtod() {
		final AdsService adsService = new AdsServiceProducer().createAdsService();

		// we can only check if nothing bad happens as the result service is a proxy
		assertThat("Ads service", adsService, is(not(nullValue())));
	}
